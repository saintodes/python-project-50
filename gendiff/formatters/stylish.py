...
def tree_parser(branch, level):
    ...
    for key, node in sorted(branch.items()):
        if node['status'] == 'unchanged':
            ...
            result.append(f"{indent_space_minus_sign}+ {key}: {format_value(node['value'], level)}\n")
        elif node['status'] == 'removed':
            result.append(f"{indent_space_minus_sign}- {key}: {format_value(node['value'], level)}\n")
        elif node['status'] == 'changed':
            result.append(f"{indent_space_minus_sign}- {key}: {format_value(node['old_value'], level)}\n")
            result.append(f"{indent_space_minus_sign}+ {key}: {format_value(node['new_value'], level)}\n")
...
def format_value(value, level):
    if isinstance(value, dict):
        ...
        for key, val in value.items():
            formatted_value.append(f"{indent_space * (level + 1)}{key}: {format_value(val, level +1)}\n")
