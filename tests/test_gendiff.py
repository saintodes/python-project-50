...
def test_stylish_flat():
    ...
    assert generate_diff(json_dict1, json_dict2, 'stylish') == open(flat_result, 'r').read()

    yml_dict1 = 'tests/fixtures/flat/file1.yml'
    yml_dict2 = 'tests/fixtures/flat/file2.yml'

    assert generate_diff(yml_dict1, yml_dict2, 'stylish') == open(flat_result, 'r').read()
...
def test_plain_nested():
    ...
    assert generate_diff(json_dict1, json_dict2, 'plain') == open(nested_result, 'r').read()
